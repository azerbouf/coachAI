...
export function UpcomingWorkoutCard({ workout }: UpcomingWorkoutCardProps) {
  ...
  if (!workout) {
    return (
      <div className="card h-full flex flex-col items-center justify-center text-center py-8">
        ...
        <p className="text-sm font-medium text-text-secondary">No upcoming workouts</p>
        <p className="text-xs text-text-muted mt-1">Training plan not yet initialized</p>
      </div>
    );
  }
  ...
  return (
    <div className="card h-full" style={{ borderColor: `${color}25` }}>
      {/* Header */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <Calendar className="w-4 h-4 text-text-muted" />
          <span className="text-sm font-medium text-text-secondary">Next Workout</span>
        </div>
        <span
          className="badge"
          style={{
      ...
      {!isRest && workout.distanceKm > 0 && (
        <div className="flex items-baseline gap-1.5 mb-2">
          <span className="stat-value text-2xl" style={{ color }}>
            {workout.distanceKm}
          </span>
      ...
      {/* Notes */}
      {workout.notes && (
        <p className="text-xs text-text-muted bg-white/5 rounded-lg px-3 py-2 mb-3 border border-border">
          {workout.notes}
        </p>
      )}
      ...
      {!isRest && (
        <button
          ...
          className="btn btn-secondary w-full text-xs mt-auto"
          style={
